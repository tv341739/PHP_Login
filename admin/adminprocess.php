...
class AdminProcess
{
   ...
   function procBanUser(){
      ...
      /* Ban user from member system */
		 ...
		 $q = "UPDATE ".TBL_USERS." SET userlevel=".BANNED_LEVEL.", timestamp='$session->time' WHERE username = '$subuser'";
         $database->query($q);
   ...
   function procDeleteBannedUser(){
      ...
      /* Delete user from database */
         ...
         $q = "DELETE FROM ".TBL_USERS." WHERE username = '$subuser' AND userlevel='".BANNED_LEVEL."'";
         $database->query($q);
         header("Location: ".$session->referrer);
      }
   }
